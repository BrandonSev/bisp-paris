...
function FamillePage() {
  ...
  useEffect(() => {
    if (profile) setFamilyName(profile.family_name || profile.nom || "");
  }, [profile]);
  ...
  const handleSaveName = async () => {
    setSavingName(true);
    try {
      await updateProfile({ family_name: familyName.trim() } as any);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-6xl px-4 pt-6 pb-10 sm:px-6 lg:px-8">
        ...
        <section className="mt-8 rounded-3xl border border-border bg-card p-6 shadow-sm">
          ...
          <div className="mt-4 flex flex-col gap-3 sm:flex-row">
            ...
            <button
              ...
              disabled={savingName}
              className="h-11 rounded-lg bg-primary px-5 text-sm font-semibold text-primary-foreground hover:bg-primary/90 disabled:opacity-50"
            >
            ...
            {children.length > 0 && (
              <ul className="mt-3 space-y-1.5">
                {children.map((c) => (
                  <li
                    key={c.id}
                    className="flex items-center gap-2.5 rounded-lg border border-border bg-background px-2.5 py-1.5"
                  >
...
function ParentCard({
  ...
  const [roleSelect, setRoleSelect] = useState(initialRoleIsKnown ? (parent.role || "Parent") : "Autre");
    ...
    <form onSubmit={handleSubmit} className="rounded-3xl border border-border bg-card p-6 shadow-sm">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 text-sm font-semibold text-foreground">
          ...
          {isDraft && (
            <span className="ml-2 rounded-full bg-primary/10 px-2 py-0.5 text-[10px] font-medium uppercase tracking-wider text-primary">
              À enregistrer
            </span>
          )}
        </div>
        ...
        <Field label="Rôle">
          <select
            ...
            onChange={(e) => {
              ...
              setRoleSelect(v);
              if (v === "Autre") set("role", roleCustom.trim() || "Autre");
              else set("role", v);
            }}
          ...
          {roleSelect === "Autre" && (
            <input
              ...
              placeholder="Précisez (Marraine, Oncle…)"
              maxLength={60}
      ...
      <div className="mt-5 space-y-3 rounded-2xl border border-border bg-muted/30 p-4">
        <label className="flex cursor-pointer flex-col items-start justify-start gap-1">
          <div className="flex items-center gap-3">
            ...
            />
            <span className="text-sm font-medium text-foreground">Adresse de livraison par défaut</span>
          </div>
        ...
        <label className="flex cursor-pointer flex-col items-start justify-start gap-1">
          <div className="flex items-center gap-3">
            ...
            />
            <span className="text-sm font-medium text-foreground">Adresse de livraison différente</span>
          </div>
          <span className="ml-7 text-xs text-muted-foreground">
            Cochez pour livrer à une autre adresse (ex. lieu de travail, autre domicile).
            {index >= 1 && " L'adresse principale sera recopiée, vous pourrez ensuite la modifier."}
          </span>
        </label>
