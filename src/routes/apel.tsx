...
export const Route = createFileRoute("/apel")({
  head: () => ({ meta: [{ title: "APEL — Suivi des commandes — BISP" }] }),
  component: () => (
    <RequireAuth>
      <ApelPage />
    </RequireAuth>
...
function ApelPage() {
  ...
  const exportCsv = () => {
    const header = ["Civilité", "Prénom", "Nom", "Email", "Téléphone", "Ville", "Nb enfants", "Classes", "Nb commandes", "Nb articles", "Dernière commande", "Statut"];
        ...
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <span className="inline-flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.18em] text-primary">
              <span className="h-px w-6 bg-primary/40" /> Espace APEL
            </span>
        ...
        {filter !== "ordered" && (
          <div className="mt-4 rounded-2xl border border-amber-300/40 bg-amber-50/40 p-4 dark:bg-amber-950/20">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Sélectionnez les familles ci-dessous puis cliquez sur « Envoyer la relance ». Vous pouvez ajouter un
            ...
            <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
              <p className="text-xs text-muted-foreground">{selected.size} famille(s) sélectionnée(s)</p>
              <button
