...
function EnfantsPage() {
  const { profile, children, removeChild } = useStore();
  const [editing, setEditing] = useState<Child | null>(null);
  const [creating, setCreating] = useState(false);
  const search = Route.useSearch();
  const navigate = useNavigate();
  const familyName = profile?.family_name || profile?.nom || "votre famille";
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <section className="relative mx-auto w-full max-w-6xl px-4 pt-6 pb-12 sm:px-6 lg:px-8">
        ...
        <div className="relative flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-2 max-w-xl text-sm leading-relaxed text-muted-foreground">
              Retrouvez ici les profils de vos enfants scolarisés à BISP. Mettez à jour leurs
              mensurations pour des tailles toujours adaptées.
            </p>
          </div>
          <button
        ...
        <div className="mt-5 flex items-start gap-2 rounded-xl border border-border bg-secondary px-4 py-3 text-xs text-muted-foreground">
          ...
          <p>Ces informations permettent d'adapter les tailles proposées dans la boutique. Données privées · private data.</p>
        </div>
          ...
          {children.length === 0 && !creating && (
            <div className="rounded-2xl border border-dashed border-border bg-card p-10 text-center">
              <p className="text-sm text-muted-foreground">
                Aucun enfant enregistré. Ajoutez votre premier enfant pour commencer.
              </p>
            </div>
          )}
          {children.map((e) => (
  ...
  return (
    ...
    >
      <div className="grid gap-0 lg:grid-cols-[280px_1fr]">
                    ...
                    <p className="text-xs text-foreground/70">
                      {enfant.genre === "Fille"
                        ...
                        : enfant.genre === "Garçon"
                          ? "Né"
                          : "Né(e)"}{" "}
                      le {new Date(enfant.naissance).toLocaleDateString("fr-FR")}
                    </p>
                    {ageInfo && (
                      <span
        ...
        <div className="p-6 sm:p-8">
                  ...
                  <TooltipProvider>
                    <Tooltip>
                      ...
                      <TooltipContent side="top" className="max-w-xs text-xs">
                        Les enfants grandissent vite : des mensurations à jour permettent de
                        proposer une taille fiable pour chaque vêtement. Vérifiez-les avant chaque
                        commande.
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>{" "}
          ...
          <div className="mt-6 flex flex-wrap items-center justify-end gap-3 border-t border-border pt-5">
            ...
            >
              <Trash2 className="h-3.5 w-3.5" /> Supprimer la fiche
            </button>
            <button
              onClick={onEdit}
      ...
      <div className="border-t border-border bg-background/60 px-6 py-4 sm:px-8">
        <PurchaseHistoryPreview childId={enfant.id} />
      </div>
    </article>
  );
}
