/**
 * Shared size recommendation logic based on the official BISP size guide.
 * Used by the size guide page, the child dialog, and the cart.
 */
...
function findRowIndexFor(
  value: number,
  key: keyof Pick<SizeRow, "stature" | "poitrine" | "taille" | "bassin">,
): number {
...
export type RecommendOptions = {
  /** "outer" applies +1 size for outer layers (pull, blouse, blazer). */
  product?: "outer";
};
...
export function recommendSize(
  ...
  if (stature !== null)
    candidates.push({ key: "Stature", idx: findRowIndexFor(stature, "stature"), value: stature });
  if (poitrine !== null)
