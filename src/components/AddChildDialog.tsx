...
export function AddChildDialog({ open, initial, onClose, onCreated }: Props) {
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.prenom || !form.nom || !form.naissance || !form.classe || !form.section || !form.hauteur || !form.tour) {
      toast.error(
        ...
        <div className="mt-4 grid gap-2.5 sm:grid-cols-4">
          ...
          <div className="sm:col-span-4 grid gap-3 sm:grid-cols-[1fr_auto] sm:items-start">
            <div className="flex flex-col gap-2.5 sm:self-center">
              <div className="rounded-xl border border-border bg-background/60 p-2.5 sm:max-w-sm">
                <SizeSelect
  ...
  return (
    ...
    >
      ...
      <span className="text-[11px] italic text-muted-foreground leading-snug">
        Conseil : complétez les 4 mensurations pour bénéficier d'une recommandation fiabilisée.{" "}
